...
from backend.app.ml.initialize_models import build_gender_model
from backend.ml_training.preprocess import get_data_pipelines
from backend.app.core.logging import logger

def train_gender_classifier(dataset_dir: str, epochs: int, batch_size: int, output_path: str):
    logger.info(f"Starting Gender Classifier training on dataset from: {dataset_dir}")
    
    # Get datasets
    try:
        train_ds, val_ds = get_data_pipelines(dataset_dir, batch_size=batch_size, task="gender")
    ...
    logger.info(f"Gender classification model training completed. Best model saved to: {output_path}")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output", 
        type=str, 
        default=os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "app", "ml", "models", "gender_model.h5")),
    ...
    train_gender_classifier(args.dataset, args.epochs, args.batch_size, args.output)
